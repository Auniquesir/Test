...
if [[ "$repo_name" == "Test" ]]; then
    echo "Running in Test repository"
    mkdir -p Test-repo/{Rule,QuantumultX/Rule,Stash/Rule,Surge/Rule}
    ...
    > Test-repo/Rule/AdBlockLite.list
    for url in "${urls[@]}"; do
        echo "Downloading: $url"
        curl -f -L "$url" >> Test-repo/Rule/AdBlockLite.list || { echo "Download Failed: $url"; exit 1; }
        echo "" >> Test-repo/Rule/AdBlockLite.list
    done
    ...
    > Test-repo/Rule/Global.list
    for url in "${urls[@]}"; do
        echo "Downloading: $url"
        curl -f -L "$url" >> Test-repo/Rule/Global.list || { echo "Download Failed: $url"; exit 1; }
        echo "" >> Test-repo/Rule/Global.list
    done
    ...
    for file in "${files[@]}"; do
        set -- $file
        echo "Downloading: $2"
        curl -f -L -o "Test-repo/Rule/$1" "$2" || { echo "Download Failed: $2"; exit 1; }
    done

    files=(
        "QuantumultX/Rule/AdBlock.list https://raw.githubusercontent.com/privacy-protection-tools/anti-AD/master/anti-ad-surge.txt"
        "Stash/Rule/AdBlock.yaml https://raw.githubusercontent.com/privacy-protection-tools/anti-AD/master/anti-ad-surge.txt"
        "Egern/Rule/AdBlock.list https://raw.githubusercontent.com/privacy-protection-tools/anti-AD/master/anti-ad-surge.txt"
        "QuantumultX/Rule/Advertising.list https://raw.githubusercontent.com/Cats-Team/AdRules/main/adrules.list"
        "Stash/Rule/Advertising.yaml https://raw.githubusercontent.com/Cats-Team/AdRules/main/adrules.list"
        "Egern/Rule/Advertising.list https://raw.githubusercontent.com/Cats-Team/AdRules/main/adrules.list"
    )
    for file in "${files[@]}"; do
        set -- $file
        echo "Downloading: $2"
        curl -f -L -o "Test-repo/$1" "$2" || { echo "Download Failed: $2"; exit 1; }
    done

    echo "Test repository: All Files Downloaded!"

# 创建当前仓库的目录结构
elif [[ "$repo_name" == "Test" ]]; then
    echo "Running in Test repository"
    mkdir -p Test-repo/{Clash,Egern,Loon,QuantumultX,Shadowrocket,Sing-box,Stash,Surge}/Rule
    ...
    for rule in "${!ruleset[@]}"; do
        for platform in "${!formats[@]}"; do
            ...
            output_dir="Test-repo/$platform/Rule"
            echo "Downloading ${ruleset[$rule]} for $platform to $output_dir/$rule.$extension"
            if ! curl -L -o "$output_dir/$rule.$extension" \
            "https://raw.githubusercontent.com/Auniquesir/Tool/X/Egern/Rules/${ruleset[$rule]}"; then
                echo "Failed to download ${ruleset[$rule]} for $platform"
            fi
        done
    done

    echo "Test repository: All Rules Downloaded!"
else
    echo "Unknown repository. Please run this script in either Test or Test repository."
    exit 1
fi
